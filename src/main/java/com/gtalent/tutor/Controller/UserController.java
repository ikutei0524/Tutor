...
public class UserController {
    ...
    public ResponseEntity<List<GetUserResponse>> searchUser(@RequestParam String keyword) {
        List<GetUserResponse> responses = mockUser.values()
                .stream()//lambda起手示(表達示)
                .filter(user ->
                        user.getUsername().toLowerCase().contains(keyword.toLowerCase()))//結果為true的user
                //所有結果為true的user mapping 成GetUserResponse
    ...
    // 將 User 轉成 GetUserResponse 的方法
    private GetUserResponse toGetUserResponse(User user) {
        return new GetUserResponse(user.getUsername(), user.getId());
    }
}
